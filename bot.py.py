...
async def start(message: Message):
    ...
    await message.answer(
        f"🎓 *IELTS BOT ga xush kelibsiz!*\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"📚 *Reading* — Grammatika testlari (⏱15 sek)\n"
        f"✍️ *Writing* — Esse yozish + avtomatik baho\n"
        f"📖 *Dictionary* — 1000+ inglizcha-o'zbekcha so'z\n"
        f"⚔️ *Duel* — Do'stingiz bilan musobaqa!\n"
        f"🎮 *Word Game* — So'z topish o'yini\n"
        f"💡 *Daily Word* — Kunlik yangi so'z\n"
        f"🏆 *Leaderboard* — Top 10 o'quvchilar\n"
        f"📊 *My Stats* — Shaxsiy statistika\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"🏅 Ball tizimi:\n"
        f"⭐ Oson savol = 1 ball\n"
...
async def help_cmd(message: Message):
    await message.answer(
        "📋 *BUYRUQLAR:*\n\n"
        "`/dict apple` — so'z tarjimasi\n"
        "`/random_word` — tasodifiy so'z\n"
        "`/duel @username` — duel chaqirish\n"
        "`/stats` — statistika\n"
        "`/streak` — ketma-ket kunlar\n\n"
        "⏱ *Har bir savol uchun 15 sekund vaqt beriladi!*\n"
...
async def send_q(message: Message, uid):
    ...
    text = f"❓ *Savol {s['index'] + 1}/{len(s['questions'])}* {pts_label}\n\n{q['q']}"
    ...
    if s["section"] in ("reading", "duel"):
        ...
        text += "\n\n_(Javobingizni yozing — kamida 3 gap)_\n⏱ *60 sekund!*"
...
async def question_timer(uid, timeout, message):
    ...
    if s["section"] == "reading":
        await message.answer(f"⏰ *Vaqt tugadi!* To'g'ri javob: *{q['a']}*", parse_mode="Markdown")
...
async def start_test(message: Message):
    ...
    await message.answer(
        f"🚀 *{section_name} testi boshlandi!*\n\n"
        f"📝 Jami: 10 ta savol\n"
...
async def check_answer(callback: CallbackQuery):
    ...
    if chosen == q["a"]:
        s["score"] += q.get("pts", 1)
        s["correct"] += 1
        await callback.answer(f"✅ To'g'ri! +{q.get('pts',1)} ball")
    else:
        s["wrong"] += 1
        await callback.answer(f"❌ Noto'g'ri! To'g'risi: {q['a']}", show_alert=True)
...
async def writing_answer(message: Message):
    ...
    feedback = f"✅ *Qabul qilindi!* ({word_count} so'z)\n"
    if word_count < 50:
...
async def finish(message: Message, uid):
    ...
    if s["section"] == "reading":
        ...
        await message.answer(
            ...
            f"❌ Noto'g'ri: *{wrong}*\n"
            f"⏰ Vaqt tugagan: *{timeouts}*\n"
            f"🏆 Ball: *{score}/{max_pts}*\n"
            f"📊 Band: *{band}*\n"
            f"🎯 Daraja: {level}\n"
        ...
        await message.answer(
            f"🏁 *Writing testi yakunlandi!*\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"✅ Yozilgan: *{answered}/{total}*\n"
            f"🏆 Umumiy ball: *{score}/{max_total}*\n"
            f"📊 Band: *{band}*\n"
            f"🎯 Daraja: *{band_label}*\n"
            f"⏱ Vaqt: {minutes} daq {seconds} sek\n"
            f"━━━━━━━━━━━━━━━━━━\n\n"
...
async def daily_word_btn(callback: CallbackQuery):
    ...
    random.seed()
    await callback.message.answer(
        f"💡 *Kunlik so'z — {time.strftime('%d.%m.%Y')}*\n\n"
        f"🔤 *{word.upper()}*\n"
        f"🇺🇿 `{dictionary[word]}`\n\n"
...
async def search_dict(message: Message):
    ...
    if len(parts) < 2:
        return await message.answer("❗ Misol: `/dict apple`", parse_mode="Markdown")
    ...
    if matches:
        result = "\n".join([f"• *{eng}* → `{dictionary[eng]}`" for eng in matches[:5]])
        await message.answer(
            f"📖 *'{word}'* uchun topildi:\n\n{result}",
            parse_mode="Markdown"
        )
    else:
...
async def daily_word(message: Message):
    ...
    await message.answer(
        ...
        f"🇺🇿 `{dictionary[word]}`\n\n"
        f"📖 *Misol:*\n"
        f"_{word.capitalize()} is an important word to know._\n\n"
        f"📌 Bu so'zni bugun 5 marta ishlating!",
        parse_mode="Markdown"
    )
...
async def my_stats(message: Message):
    ...
    await message.answer(
        ...
        f"🏆 Umumiy ball: *{score}*\n"
        f"📝 Testlar soni: *{total_tests}*\n"
        f"📈 O'rtacha ball: *{avg}*\n"
        f"❌ Xatolar: *{mistake_count}*\n"
        f"🥇 Reyting: *#{rank_above}*\n"
        f"━━━━━━━━━━━━━━━━━━",
        parse_mode="Markdown"
    )
...
async def duel_menu(message: Message):
    await message.answer(
        "⚔️ *DUEL — Do'stingiz bilan musobaqa!*\n\n"
        "Duel boshlash uchun:\n"
        "👉 `/duel @username`\n\n"
        "Misol: `/duel @ali_123`\n\n"
        "📋 Qoidalar:\n"
...
async def start_duel(message: Message):
    ...
    if len(parts) < 2:
        return await message.answer("❗ Misol: `/duel @username`", parse_mode="Markdown")
    ...
    cur.execute(
        ...
        (duel_id, uid, 0, 0, 0, 0, 0, questions_json, "waiting")
    )
    conn.commit()

    duel_waiting[opponent_username.lower()] = {
        "duel_id": duel_id,
        "challenger_id": uid,
        "challenger_name": my_username,
        "questions": selected
    }

    await message.answer(
        f"⚔️ *Duel chaqiruvi yuborildi!*\n\n"
        f"📨 @{opponent_username} ga taklif yuborildi.\n"
        f"Ular `/accept_duel` buyrug'ini bosishlari kerak.\n\n"
        f"⏳ 5 daqiqa kutiladi...",
        parse_mode="Markdown"
    )

    # Notify the opponent if possible
    try:
        challenger_name = message.from_user.full_name
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚔️ Qabul qilish", callback_data=f"accept_duel_{duel_id}_{uid}")]
        ])
        # We can't directly message them without their ID, so we store it
        await message.answer(
            f"💡 @{opponent_username} sizni ko'rishi uchun ular botga `/accept_duel` yozishlari kerak!",
        )
    except:
        pass

@dp.message(F.text.startswith("/accept_duel"))
async def accept_duel_cmd(message: Message):
    uid = message.from_user.id
    username = (message.from_user.username or "").lower()

    duel_info = duel_waiting.get(username)
    if not duel_info:
        return await message.answer(
            "❌ Sizga duel chaqiruvi yo'q.\n"
            "Do'stingizdan `/duel @sizning_username` yuborishini so'rang!"
        )
    ...
    cur.execute("UPDATE duels SET player2 = ?, status = 'active' WHERE duel_id = ?", (uid, duel_id))
    conn.commit()

    del duel_waiting[username]

    # Start duel for both players
    for player_uid in [challenger_id, uid]:
        state[player_uid] = {
            "section": "duel",
            "index": 0,
            "questions": questions,
            ...
            "start_time": time.time(),
            "duel_id": duel_id,
            "opponent_id": uid if player_uid == challenger_id else challenger_id,
            "correct": 0,
            "wrong": 0,
            "timeouts": 0,
        }
    ...
    try:
        await bot.send_message(
            challenger_id,
            f"⚔️ *DUEL BOSHLANDI!*\n\n"
            f"👤 Raqib: @{message.from_user.username or 'Raqib'}\n"
            f"📝 10 ta savol | ⏱ 15 sek/savol\n\n"
            f"🥊 Omad!",
            parse_mode="Markdown"
        )
    ...
    await send_duel_q(message, uid)
    try:
        fake_msg = type('obj', (object,), {'answer': lambda self, *a, **kw: bot.send_message(challenger_id, *a, **kw), 'chat': type('c', (), {'id': challenger_id})()})()
...
async def send_duel_q_by_id(uid):
    ...
    pts_label = get_pts_label(q.get("pts", 1))
    text = (
        f"⚔️ *DUEL — Savol {s['index']+1}/10* {pts_label}\n\n"
        f"{q['q']}\n\n"
        + "\n".join(q["options"])
        + "\n\n⏱ *15 sekund!*"
    )
...
async def duel_timer(uid, timeout):
    ...
    await bot.send_message(uid, f"⏰ *Vaqt tugadi!* To'g'ri: *{q['a']}*", parse_mode="Markdown")
...
async def duel_answer(callback: CallbackQuery):
    ...
    if chosen == q["a"]:
        s["score"] += q.get("pts", 1)
        s["correct"] += 1
        await callback.answer(f"✅ To'g'ri! +{q.get('pts',1)}")
    else:
        s["wrong"] += 1
        await callback.answer(f"❌ Noto'g'ri! To'g'risi: {q['a']}", show_alert=True)
...
async def finish_duel(uid):
    ...
    if row2 and row2[2] == 1 and row2[3] == 1:
        ...
        if p1_score > p2_score:
            ...
            for pid in [p1, p2]:
                try:
                    await bot.send_message(pid, "🤝 *Duel — Durang!*\nIkkovingiz teng ball to'pladingiz!", parse_mode="Markdown")
                except:
                    pass
            for pid in [p1, p2]:
                state.pop(pid, None)
        ...
        try:
            await bot.send_message(
                winner_id,
                f"🏆 *DUEL G'OLIBI SIZMISIZ!*\n\n"
                f"✅ Sizning ball: *{w_score}*\n"
                f"❌ Raqib ball: *{l_score}*\n\n"
        ...
        try:
            await bot.send_message(
                loser_id,
                f"😔 *Duelda yutqazdingiz...*\n\n"
                f"✅ Sizning ball: *{l_score}*\n"
                f"🏆 Raqib ball: *{w_score}*\n\n"
...
async def word_game(message: Message):
    ...
    await message.answer(
        ...
        f"🔀 Aralashtirilgan harf: `{scrambled}`\n\n"
        f"💡 Maslahat: {hint}\n\n"
